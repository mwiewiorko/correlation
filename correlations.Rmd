...
Function to calculta cor between all variable type (num, cat)
```{r}
###############################################################################
#
# cor2
...
Load Data
```{r cars}
cred_app <- read_csv("~/2. Rozwojowe kwestie/0_Data/1_home_credit_default_risk/application_train.csv")
glimpse(cred_app)
```

Calculate Cor
```{r}
a<-cred_app[,1:10] %>% cor2
```


```{r}
col <- colorRampPalette(c("#BB4444", "#EE9988", "#FFFFFF", "#77AADD", "#4477AA"))
a %>% corrplot(method="circle", col=col(200),  
         type="upper",  
         addCoef.col = "black", # Add coefficient of correlation
         tl.col="black",tl.cex=.5, tl.srt=45, number.cex = .9,#Text label color and rotation
         diag=FALSE)
